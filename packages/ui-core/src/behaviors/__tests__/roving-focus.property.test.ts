...
describe('Roving Focus Property-Based Tests', () => {
  ...
  it('Property 31: Focus change callback invocation', () => {
    fc.assert(
      fc.property(
        ...
        fc.array(fc.string({ minLength: 1 }), { minLength: 2, maxLength: 10 }).map((items, idx) => 
          items.map((item, i) => `${item}-${idx}-${i}`) // Ensure unique IDs
        ),
  ...
  it('Property 31d: Callback invoked with correct previousIndex after multiple moves', () => {
    fc.assert(
      fc.property(
        fc.array(fc.string({ minLength: 1 }), { minLength: 3, maxLength: 10 }).map((items, idx) => 
          items.map((item, i) => `${item}-${idx}-${i}`)
        ),
        (items) => {
          const onFocusChange = vi.fn();
  ...
  it('Property 31e: Callback invoked when setItems causes index adjustment', () => {
    fc.assert(
      fc.property(
        fc.array(fc.string({ minLength: 1 }), { minLength: 5, maxLength: 10 }).map((items, idx) => 
          items.map((item, i) => `${item}-${idx}-${i}`)
        ),
        fc.array(fc.string({ minLength: 1 }), { minLength: 1, maxLength: 3 }).map((items, idx) => 
          items.map((item, i) => `new-${item}-${idx}-${i}`)
        ),
