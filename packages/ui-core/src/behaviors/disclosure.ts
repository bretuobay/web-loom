 ...
 */
export function createDisclosureBehavior(options?: DisclosureBehaviorOptions): DisclosureBehavior {
  const initialState: DisclosureState = {
    isExpanded: options?.initialExpanded ?? false,
    id: options?.id || null,
  };

  const store: Store<DisclosureState, DisclosureActions> = createStore<DisclosureState, DisclosureActions>(
    initialState,
    (set, get) => ({
      expand: () => {
