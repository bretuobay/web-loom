...
describe('Property 23: Resource cleanup on watch exit', () => {
  ...
  it('should close browser manager when cleanup is called', async () => {
    await fc.assert(
      fc.asyncProperty(fc.integer({ min: 1, max: 10 }), async (numOperations) => {
  ...
  it('should close file watcher when cleanup is called', () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 10 }), (numChanges) => {
  ...
  it('should prevent multiple cleanup calls with idempotency flag', () => {
    fc.assert(
      fc.property(fc.integer({ min: 2, max: 10 }), (numCleanupAttempts) => {
  ...
  it('should continue cleanup even if one component fails', async () => {
    await fc.assert(
      fc.asyncProperty(fc.constantFrom('watcher', 'browser'), async (failingComponent) => {
        const cleanupOrder: string[] = [];
        const errors: Error[] = [];
