...
describe('Property 34: Page cleanup after capture', () => {
  ...
  it(
    'should reduce active page count after releasing pages',
    async () => {
      await fc.assert(
  ...
  it(
    'should handle page cleanup even if page is already closed',
    async () => {
      await fc.assert(
  ...
  it(
    'should pool pages for reuse after release',
    async () => {
      await fc.assert(
  ...
  it(
    'should navigate pages to about:blank when releasing',
    async () => {
      await fc.assert(
        fc.asyncProperty(fc.integer({ min: 1, max: 3 }), async (numPages) => {
