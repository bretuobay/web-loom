...
describe('Hub & Spoke Navigation - Property-Based Tests', () => {
   ...
   */
  const spokesArrayArbitrary = fc.array(spokeArbitrary, { minLength: 1, maxLength: 10 }).map((spokes) => {
    // Ensure unique IDs
  ...
  it('Property 18: Navigation history consistency', () => {
    fc.assert(
      fc.property(spokesArrayArbitrary, fc.array(fc.nat(), { minLength: 1, maxLength: 5 }), (spokes, indices) => {
        const navigation = createHubAndSpoke({ spokes });
  ...
  it('Property: Breadcrumb accumulation on multiple activations', () => {
    fc.assert(
      fc.property(spokesArrayArbitrary, fc.array(fc.nat(), { minLength: 2, maxLength: 5 }), (spokes, indices) => {
        const navigation = createHubAndSpoke({ spokes });
  ...
  it('Property: goBack navigates through history correctly', () => {
    fc.assert(
      fc.property(spokesArrayArbitrary, fc.array(fc.nat(), { minLength: 2, maxLength: 4 }), (spokes, indices) => {
