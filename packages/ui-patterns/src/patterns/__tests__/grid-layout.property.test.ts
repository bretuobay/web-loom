...
describe('Grid Layout - Property-Based Tests', () => {
   ...
   */
  const itemsArrayArbitrary = fc
    .array(testItemArbitrary, { minLength: 1, maxLength: 20 })
    .map(items => {
      // Ensure unique IDs
   ...
   */
  const breakpointsArrayArbitrary = fc
    .array(breakpointArbitrary, { minLength: 1, maxLength: 5 })
    .map(breakpoints => {
