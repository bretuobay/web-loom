...
export async function runNotificationDemo() {
    ...
    notificationService.notifications$.subscribe((notifications: Notification[]) => {
      if (notifications.length > 0) {
        ...
        notifications.forEach((n) => {
          console.log(
            `  [${n.type.toUpperCase()}] (ID: ${n.id}) ${n.message} ${n.isPersistent ? '(Persistent)' : `(Dismisses in ${n.duration}ms)`}`,
          );
        });
      } else {
  ...
  const persistentInfoId = notificationService.showPersistentInfo(
    'This is a persistent informational message. It needs manual dismissal.',
  );
  await new Promise((resolve) => setTimeout(resolve, 500));
