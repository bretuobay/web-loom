...
//  fake import definition for CacheProvider
class CacheProvider {
  get<T>(key: string): Promise<{ data: T; lastUpdated: number } | null> {
    // Simulate cache get operation
    return Promise.resolve(null); // No data found in cache
  }
  set<T>(key: string, value: { data: T; lastUpdated: number }): Promise<void> {
    // Simulate cache set operation
    return Promise.resolve(); // Successfully cached
  }
  remove(key: string): Promise<void> {
    // Simulate cache remove operation
    return Promise.resolve(); // Successfully removed from cache
  }
  // Additional methods can be added as needed
}
const LocalStorageCacheProvider = CacheProvider;
const IndexedDBCacheProvider = CacheProvider;
const InMemoryCacheProvider = CacheProvider;
...
class QueryCore {
  ...
  public async refetch<TData>(endpointKey: string, forceRefetch = false): Promise<void> {
    ...
    if (!forceRefetch && refetchAfter && lastUpdated) {
      ...
      if (timeSinceLastFetch < refetchAfter) {
        console.log(
          `QueryCore: Refetch for "${endpointKey}" skipped, data is still fresh (fetched ${
            timeSinceLastFetch / 1000
          }s ago, refetchAfter ${refetchAfter / 1000}s).`,
        );
