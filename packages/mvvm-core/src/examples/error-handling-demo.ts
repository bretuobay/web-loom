...
export async function runErrorHandlingDemo() {
  ...
  await firstValueFromSafe(
    riskyService.doSomethingRisky(true).pipe(
      catchError((err) => {
  ...
  await firstValueFromSafe(
    riskyService.doSomethingRisky(false).pipe(
      catchError((err) => {
  ...
  try {
    return await new Promise((resolve, reject) => {
      ...
      sub = obs.subscribe({
        ...
        complete: () => {
          // In case observable completes without a value after an error
          resolve(undefined);
          if (sub) sub.unsubscribe();
        },
      });
...
if (require.main === module) {
  runErrorHandlingDemo().catch((err) => {
    ...
    const ges = SimpleDIContainer.isRegistered('globalErrorService')
      ? SimpleDIContainer.resolve('globalErrorService')
      : null;
