...
export function registerDebugViewModelPrompt(server: McpServer): void {
  server.registerPrompt(
    ...
    {
      description:
        'Diagnose common issues in a web-loom ViewModel — missing dispose, leaked subscriptions, incorrect canExecute wiring, UI state in Model.',
      argsSchema: {
        ...
        symptom: z
          ...
          .optional()
          .describe(
            "Describe the problem you are seeing (e.g. 'memory leak', 'command never enables', 'data not updating')",
          ),
      },
    },
    async ({ code, symptom }) => ({
