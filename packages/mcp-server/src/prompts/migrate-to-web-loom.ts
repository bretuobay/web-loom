...
export function registerMigrateToWebLoomPrompt(server: McpServer): void {
  server.registerPrompt(
    ...
    {
      description:
        'Step-by-step migration guide from plain framework code (React useState/useEffect, Vue refs, vanilla JS) to the web-loom MVVM pattern.',
      argsSchema: {
        code: z.string().describe('The existing code to migrate (React component, Vue SFC, plain JS module, etc.)'),
        framework: z.enum(['react', 'vue', 'vanilla', 'angular', 'lit']).describe('The source framework of the code'),
      },
    },
