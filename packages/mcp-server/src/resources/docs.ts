...
const PACKAGES = [
  {
    pkg: 'mvvm-core',
    description:
      'Core MVVM library — BaseModel, BaseViewModel, RestfulApiModel, Command, CompositeCommand, ObservableCollection',
  },
  ...
  {
    pkg: 'ui-core',
    description: 'Headless UI behaviors — Dialog, Form, ListSelection, RovingFocus, DragDrop, UndoRedo',
  },
...
export function registerDocsResources(server: McpServer): void {
  for (const { pkg, description } of PACKAGES) {
    ...
    server.registerResource(`docs-${pkg}`, uri, { description, mimeType: 'text/markdown' }, async (_uri) => {
