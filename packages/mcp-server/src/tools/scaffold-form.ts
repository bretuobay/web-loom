...
export function registerScaffoldFormTool(server: McpServer): void {
  server.registerTool(
    ...
    {
      description:
        'Generate a forms-core form setup with Zod validation schema, field registration, and submit handler. Requires @web-loom/forms-core.',
      inputSchema: {
        name: z.string().describe("Form name in PascalCase (e.g. 'LoginForm', 'ProductCreate')"),
        fields: z.array(
          z.object({
