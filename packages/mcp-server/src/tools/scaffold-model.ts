...
const FieldSchema = z.object({
  ...
  type: z.enum(['string', 'number', 'boolean', 'string[]', 'number[]']).describe('Field type'),
...
export function registerScaffoldModelTool(server: McpServer): void {
  server.registerTool(
    ...
    {
      description:
        'Generate a web-loom Model with a Zod schema. Supports REST classes, REST config for factories, simple BaseModel state, and QueryCore cached lists.',
      inputSchema: {
        name: z.string().describe("Entity name in PascalCase (e.g. 'Product', 'UserProfile')"),
        endpoint: z.string().describe("REST API endpoint path (e.g. '/products', '/users')"),
        fields: z.array(FieldSchema).describe("Entity fields (excluding 'id' which is always included)"),
        apiBase: z.string().optional().describe("Base URL for the API (default: 'http://localhost:3000')"),
        style: z.enum(MODEL_STYLES).optional().describe("Model template style (default: 'restful-class')"),
      },
    },
