    ...
    recommendations: [
      {
        ...
        reason:
          'RestfulApiModel handles CRUD HTTP calls; RestfulApiViewModel provides fetchCommand, createCommand, updateCommand, deleteCommand out of the box.',
    ...
    recommendations: [
      {
        ...
        reason:
          'QueryCore implements stale-while-revalidate caching, background refetch, and request deduplication without a framework.',
    ...
    recommendations: [
      {
        ...
        reason:
          'createStore is designed for UI-only state (theme, sidebar, modals). Business data (entities) belongs in Models, not Store.',
        example:
          "const uiStore = createStore({ theme: 'light', sidebarOpen: true }, (setState) => ({ toggleTheme: () => setState(s => ({ ...s, theme: s.theme === 'light' ? 'dark' : 'light' })) }));",
      },
    ],
  },
  {
    ...
    recommendations: [
      {
        ...
        reason:
          'FormFactory.create() handles field lifecycle, Zod validation, async validators, computed fields, and field visibility — framework-agnostic.',
      },
      {
    ...
    recommendations: [
      {
        ...
        reason:
          'Headless behavior factories: createDialogBehavior, createDisclosureBehavior, createListSelection, createRovingFocus, createDragDropBehavior, createUndoRedoStack, createKeyboardShortcuts. No styling, no framework deps.',
      },
    ],
  },
  {
    ...
    recommendations: [
      {
        ...
        reason:
          'PluginRegistry manages declarative plugin manifests with routes, menu items, and widgets. PluginModule provides init/mount/unmount lifecycle hooks.',
      },
    ],
  },
  {
    ...
    recommendations: [
      {
        ...
        reason:
          'Storage abstraction that decouples code from direct localStorage/sessionStorage calls — easier to test and swap.',
      },
      {
        package: '@web-loom/store-core',
        reason:
          'PersistedStore wraps createStore with automatic persistence via LocalStorageAdapter or IndexedDBAdapter.',
      },
    ],
  },
  {
    ...
    recommendations: [
      {
        ...
        reason:
          'HTTP client utilities with interceptor support for auth headers, error handling, and request/response transformation.',
      },
    ],
  },
  {
    ...
    recommendations: [
      {
        package: '@web-loom/mvvm-core',
        reason:
          'Command<TParam, TResult> encapsulates async actions with isExecuting$, canExecute$, and executeError$ signals. CompositeCommand orchestrates multiple commands.',
    ...
    recommendations: [
      {
        ...
        reason:
          'signals-core provides signal(), computed(), observe(), and ReadonlySignal for framework adapters and derived reactive state.',
...
export function registerSelectPackageTool(server: McpServer): void {
  server.registerTool(
    ...
    {
      description:
        'Given a description of what you need to build, recommend the right @web-loom/* package(s) with reasoning.',
      inputSchema: {
        use_case: z
          .string()
          .describe(
