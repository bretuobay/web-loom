      ...
      if (!satisfies(VENDURE_VERSION, compatibility, { loose: true })) {
        if (ignoredPlugins.includes(plugin)) {
          ...
          throw new InternalServerError(
            `Plugin "${pluginName}" incompatible. ` + `Requires "${compatibility}", current is "${VENDURE_VERSION}"`,
          );
        }
      }
    }
...
async function runPluginConfigurations(config: RuntimeVendureConfig) {
  ...
  const results = await Promise.all(independent.map((p) => getConfigurationFunction(p)(config)));
...
_Analyzed version: Vendure v3.5.1_
_Analysis model: Claude Sonnet 4.5_
