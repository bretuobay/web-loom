...
sdk.routes.add({
  ...
  componentFactory: (sdk) => {
    ...
    return sdk.system.platform.isBrowser
      ? BrowserDashboard
      : ServerDashboard;
  },
});
...
**Vite** demonstrates that extending existing interfaces (Rollup) beats creating new ones, and filtering prevents unnecessary plugin invocations.

**Babel** shows how single-traversal multi-plugin execution optimizes build-time plugin systems.
...
**Kibana** illustrates how topological sorting and contract-based dependency injection create type-safe plugin graphs.
