...
```typescript
import { signal, effect, type ReadonlySignal } from '@web-loom/signals-core';
import { getTokenValue } from '@web-loom/design-core/utils';

class SensorViewModel extends BaseViewModel {
  private readonly _statusColor = signal<string | null>(null);
  readonly statusColor$: ReadonlySignal<string | null> = this._statusColor.asReadonly();

  constructor(model: SensorModel) {
    super(model);

    // getTokenValue is async, so we resolve it in an effect and push the
    // result into a signal rather than trying to derive it with computed()
    const statusColorEffect = effect(() => {
      const reading = this.reading$.get();
      const tokenKey =
        reading.temperature > this.criticalThreshold
          ? 'color.sensor.temperature.critical'
          : reading.temperature > this.warningThreshold
            ? 'color.sensor.temperature.warning'
            : 'color.sensor.temperature.normal';

      getTokenValue(tokenKey).then((color) => this._statusColor.set(color));
    });
    this.addSubscription(() => statusColorEffect.dispose());
  }
}
```
