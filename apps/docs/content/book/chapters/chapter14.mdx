...
export class SensorModel {
  private _data = signal<Sensor | null>(null);
  readonly data$: ReadonlySignal<Sensor | null> = this._data.asReadonly();
...
export class GreenHouseViewModel {
  ...
  async addSensor(sensorData: CreateSensorDTO): Promise<void> {
    try {
      ...
      // Update local state
      const greenhouse = this._greenhouse.peek();
      if (greenhouse) {
        greenhouse.sensors.push(sensor);
        this._greenhouse.set(greenhouse);
      }
