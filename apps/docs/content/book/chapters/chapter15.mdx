...
export class SensorViewModel {
  ...
  private _sensors = signal<Sensor[]>([]);
  private _loading = signal<boolean>(true);
  private _error = signal<Error | null>(null);

  readonly sensors$: ReadonlySignal<Sensor[]>;
  ...
  constructor(
    ...
    this.sensors$ = this._sensors.asReadonly();
    this.isLoading$ = this._loading.asReadonly();
  ...
  private async initializeQuery(): Promise<void> {
    ...
    this.queryCore.subscribe(`sensors-${this.greenhouseId}`, (state) => {
      ...
      this._error.set(state.error || null);

      if (state.data) {
        this._sensors.set(state.data);
      }
    });
  }
  ...
  dispose(): void {
    // Signals need no explicit teardown; the QueryCore subscription above
    // is what would need unsubscribing in a longer-lived ViewModel.
  }
}
```
...
1. **Encapsulates QueryCore**: The View doesn't know about QueryCore, it only sees `ReadonlySignal`s
...
4. **Handles state mapping**: Converts QueryCore's state to signals for consistency with other ViewModels
...
export class GreenhouseViewModel {
  private queryCore: QueryCore;
  private _greenhouse = signal<Greenhouse | null>(null);
  private _sensors = signal<Sensor[]>([]);
  private _loading = signal<boolean>(true);

  readonly greenhouse$: ReadonlySignal<Greenhouse | null>;
  readonly sensors$: ReadonlySignal<Sensor[]>;
  readonly isLoading$: ReadonlySignal<boolean>;
  ...
  constructor(
    ...
    this.greenhouse$ = this._greenhouse.asReadonly();
    this.sensors$ = this._sensors.asReadonly();
    this.isLoading$ = this._loading.asReadonly();
  ...
  private async initializeQueries(): Promise<void> {
    ...
    this.queryCore.subscribe(`greenhouse-${this.greenhouseId}`, (state) => {
      if (state.data) {
        this._greenhouse.set(state.data);
      }
    });
  ...
  dispose(): void {
    // Signals need no explicit teardown here beyond unsubscribing from
    // QueryCore, which this simplified example omits for brevity.
  }
}
```
...
function GreenhouseDashboard({ greenhouseId }: Props) {
  ...
  const greenhouse = useSignal(viewModel.greenhouse$);
...
export class SensorViewModel {
  async updateSensorName(sensorId: string, newName: string): Promise<void> {
    ...
    try {
      ...
      // Rollback on error
      this._sensors.set(currentSensors);
      throw error;
    }
  }
}
      ...
      if (state.data) {
        ...
        this.queryCore.subscribe(`readings-${this.sensorId}`, (readingsState) => {
          if (readingsState.data) {
            this._readings.set(readingsState.data);
          }
        });
      }
    });
...
export class SensorHistoryViewModel {
  private _pages = signal<SensorReading[][]>([]);
  private _hasMore = signal<boolean>(true);

  readonly allReadings$: ReadonlySignal<SensorReading[]>;
  ...
  constructor(
    ...
    this.allReadings$ = computed(() => this._pages.get().flat());
    this.hasMore$ = this._hasMore.asReadonly();
  }

  async loadNextPage(): Promise<void> {
    const currentPages = this._pages.peek();
...
describe('SensorViewModel', () => {
  ...
  it('loads sensors on initialization', async () => {
    ...
    const sensors = viewModel.sensors$.peek();
  ...
  it('handles errors gracefully', async () => {
    ...
    const error = viewModel.error$.peek();
