...
function SensorDashboard() {
  ...
  const sensors = useSignal(viewModel.sensors$);
  const loading = useSignal(viewModel.isLoading$);
...
describe('BaseModel', () => {
  ...
  it('should emit changes to data$ when setData is called multiple times', () => {
    ...
    // observe() delivers the current value immediately, then every future
    // change — the signals-core equivalent of a BehaviorSubject subscription.
    ...
    expect(emittedData).toEqual([
      null, // Initial state, delivered immediately by observe()
      { id: 'a', name: 'A', age: 1 },
      { id: 'b', name: 'B', age: 2 },
      null,
    ]);
...
1. **Test reactive state**: Read signals synchronously with `.get()`—no async wrapper needed
2. **Test validation**: Verify Zod schema validation works correctly
3. **Test state transitions**: Verify `setData`, `setLoading`, `setError` update the underlying signals
4. **Test edge cases**: Invalid data, null data, schema violations

### Testing Model Disposal

Models must stop notifying subscribers when disposed. Here's the disposal test:
...
**This test verifies:**
- Setters become no-ops once `dispose()` is called, so subscribers get no further notifications
- Signal values remain synchronously readable (via `.get()`) even after disposal
- No special "completion" event is needed—signals don't have a completed state, they just stop changing
...
2. **Test signal exposure**: Verify ViewModel exposes Model signals correctly, read synchronously via `.get()`
3. **Test derived state**: Verify `validationErrors$` derives from `error$` via `computed()`
4. **Test teardown registration**: Verify `addSubscription`-registered teardowns run on `dispose()`
5. **Clean up after tests**: Always call `dispose()` in `afterEach`
...
describe('fetchCommand', () => {
  it('should call model.fetch without ID when executed without parameter', async () => {
    ...
    // observe() delivers the current value immediately, then every future
    // change — so this captures the full false → true → false transition.
    observe(viewModel.isLoading$, (val) => loadingStates.push(val));

    const dataStates: (ItemArray | null)[] = [];
    observe(viewModel.data$, (val) => dataStates.push(val));
  ...
  it('should call model.fetch with ID when executed with a string parameter', async () => {
    ...
    expect(viewModel.data$.get()).toEqual({
      id: 'item-id-3',
  ...
  it('should set error$ if fetch fails', async () => {
    ...
    expect(viewModel.error$.get()).toBe(fetchError);
    expect(viewModel.isLoading$.get()).toBe(false);
  });
});
```
...
describe('createCommand', () => {
  ...
  it('should call model.create and update data', async () => {
    ...
    expect(viewModel.createCommand.isExecuting$.get()).toBe(false);
    const data = viewModel.data$.get();
...
describe('updateCommand', () => {
  ...
  beforeEach(() => {
    mockModel.setData([existingItem]);
  });
...
describe('deleteCommand', () => {
  ...
  beforeEach(() => {
    mockModel.setData([itemToDelete, { id: '2', name: 'Keep Me' }]);
  });
...
- Selection resolves to null when ID is not found
- Selection can be explicitly cleared with `null`
- Because `selectedItem$` is a `computed()`, every assertion is a plain synchronous `.get()`—no waiting for an emission is needed
...
describe('RestfulApiViewModel with Real RestfulApiModel Integration', () => {
  ...
  it('should fetch data and update viewModel.data$', async () => {
    ...
    // observe() delivers the current value immediately, then every future
    // change, so these arrays capture the full transition sequence.
...
**3. Synchronous assertions on signals:**

Because `selectedItem$` is a `computed()` signal, its value updates the moment `selectItem()` runs—no `waitFor` or async wrapper needed:
...
`vi.waitFor` is still useful for genuinely async conditions—e.g. waiting on a `setTimeout`-driven side effect—but reading a signal's current value is never one of them.
...
- Call methods and verify signal changes
- Test validation with valid and invalid data
- Verify disposal stops further notifications
...
- Verify ViewModel exposes correct signals
...
it('should call calibrateSensor when button clicked', async () => {
  ...
  const mockViewModel = {
    sensors$: signal([{ id: '1', name: 'Sensor 1' }]).asReadonly(),
    isLoading$: signal(false).asReadonly(),
    error$: signal(null).asReadonly(),
...
function SensorDashboard({ viewModel }: { viewModel: SensorViewModel }) {
  const sensors = useSignal(viewModel.data$);
  const loading = useSignal(viewModel.isLoading$);
  const error = useSignal(viewModel.error$);
  const selectedSensor = useSignal(viewModel.selectedItem$);
...
4. Verify signal changes with `.get()`
...
### Pattern 2: Read Signals Directly, or observe() a Sequence

```typescript
import { observe } from '@web-loom/signals-core';

// Get current value — no operator needed, just read it
const currentValue = viewModel.data$.get();

// Collect the current value plus every future change
const emissions: boolean[] = [];
observe(viewModel.isLoading$, val => emissions.push(val));

// If you only want *future* changes (skipping the value at subscribe time),
// use .subscribe() instead of observe()
const futureOnly: boolean[] = [];
viewModel.isLoading$.subscribe(val => futureOnly.push(val));
```

**Why:** Signals collapse what used to require `first()`, `skip()`, `take()`, and `firstValueFrom()` into two plain methods: `.get()` for a synchronous read, and `observe()`/`.subscribe()` for a sequence of changes—depending on whether you want the current value included.
...
it('should transition through loading states', async () => {
  const loadingStates: boolean[] = [];
  // observe() delivers the current value immediately, then every future
  // change — so the array below captures the full transition.
  observe(viewModel.isLoading$, val => loadingStates.push(val));
...
**Why:** State transitions reveal bugs that final state alone might miss. Note that if you use plain `.subscribe()` instead of `observe()`, the initial `false` won't be captured (subscribe only fires on future changes)—the expected sequence becomes `[true, false]`.
...
describe('SensorViewModel', () => {
  ...
  describe('loadSensors', () => {
    it('should fetch sensors for a greenhouse', async () => {
      ...
      expect(viewModel.data$.get()).toEqual(sensors);
    });
    ...
    it('should handle fetch errors', async () => {
      ...
      expect(viewModel.error$.get()).toBe(error);
    });
  });
  ...
  describe('selectSensor', () => {
    it('should select a sensor by ID', async () => {
      ...
      expect(viewModel.selectedSensor$.get()).toEqual(sensors[1]);
    });
    ...
    it('should clear selection when null is passed', async () => {
      ...
      expect(viewModel.selectedSensor$.get()).toEqual(sensors[0]);

      viewModel.selectSensor(null);

      expect(viewModel.selectedSensor$.get()).toBeNull();
    });
  });
});
```
...
3. **Test ViewModels in isolation** by mocking Models. Verify that ViewModels correctly expose Model signals, handle commands, manage selection state, and clean up on disposal.
...
9. **Signals make observable testing nearly trivial**. `.get()` reads the current value synchronously; `observe()` captures the current value plus every future change; plain `.subscribe()` captures only future changes. No operator library required.
