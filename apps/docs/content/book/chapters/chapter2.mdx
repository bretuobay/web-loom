...
This ViewModel is **completely framework-agnostic**. It doesn't import React, Vue, or any UI framework. It exposes reactive signals (`data$`, `isLoading$`, `error$`, all `ReadonlySignal<T>` from `@web-loom/signals-core`) that any framework can consume.
...
- Same `data$` and `isLoading$` signals
- Same `fetchCommand.execute()` method
- Zero business logic duplication

**What's different:**
- Only the UI rendering (JSX vs Vue template)
- Framework-specific subscription patterns (`useSignal` hook)
...
test('SensorViewModel loads sensor data', async () => {
  ...
  observe(viewModel.data$, data => dataValues.push(data));
...
MVVM uses reactive signals at its core. Here's the `BaseModel` that powers all GreenWatch models:
...
**Key features:**
- **`signal()`**: Holds current state and can be read synchronously with `.get()`/`.peek()`
- **`ReadonlySignal`**: Read-only view that Views subscribe to (via `.subscribe()`) or read (via `.get()`)
- **Automatic updates**: When `setData()` is called, all subscribers update automatically

### Framework-Specific Subscription Patterns

Each framework reads these signals using its own patterns:
...
Both hooks do the same thing—read a signal and keep local state in sync—but use framework-specific APIs. The ViewModel remains unchanged.
...
4. **Reactive state**: Signals provide automatic UI updates
...
- `apps/mvvm-react/src/hooks/useSignal.ts` - React signal hook
- `apps/mvvm-vue/src/hooks/useSignal.ts` - Vue signal composable
