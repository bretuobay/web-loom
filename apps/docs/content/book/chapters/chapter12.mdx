...
In the previous four chapters, we explored MVVM implementations in React, Vue, Angular, and Lit—each demonstrating how ViewModels integrate with different frameworks and libraries. React used a `useSignal` hook, Vue used a `useSignal` composable, Angular used the `fromLoomSignal` bridge, and Lit called `observe()` directly from web component lifecycle callbacks. Now we arrive at the ultimate proof of framework independence: **Vanilla JavaScript with no framework at all**.
...
1. **Direct Signal Subscriptions**: Call `observe()` from `@web-loom/signals-core` directly on ViewModel signals
...
4. **Subscription Cleanup**: Track and unsubscribe from signals to prevent memory leaks
...
This state object serves as a cache of the latest data from ViewModels. When ViewModel signals change, we update this state and trigger re-renders.

**Why We Need This**:

- **Synchronous Access**: EJS templates need synchronous data access; reading `.peek()` on demand would work too, but caching the last-seen value in one place keeps the rendering code simple
...
## 12.4 Direct Signal Subscriptions

The heart of the vanilla JavaScript MVVM implementation is the subscription module. This is where we call `observe()` directly on ViewModel signals and update the DOM when data changes:
...
```
We call `observe()` directly on the ViewModel's `data$` signal. No hooks, no composables, no bridge utility—just the raw `@web-loom/signals-core` function. `observe()` invokes the callback immediately with the current value, then again on every future change, so the dashboard renders right away with whatever data is already there (even before the first fetch completes) and again every time it changes. This is the most explicit form of ViewModel consumption we've seen.
...
| **Signal Subscription** | `useSignal` hook | `useSignal` composable | `fromLoomSignal` bridge | `connectedCallback` + `observe()` | Direct `observe()` |
| **State Management** | `useState`/`useSyncExternalStore` | `shallowRef` | native Angular `Signal` | `@state` decorator | Plain object |
| **Re-render Trigger** | React re-render | Ref assignment | Signal `.set()` | Property assignment | Manual `innerHTML` |
| **Subscription Cleanup** | Managed by React | `onUnmounted` | `DestroyRef` | `disconnectedCallback` | Manual (see 12.11) |
...
| **Learning Curve** | Hooks concepts | Composition API | DI + native Signals | Web Components | DOM APIs |
...
2. **Subscription Patterns Vary**: Each framework has its own way to bridge signals into its rendering model, but they all achieve the same result—reactive UI updates—with a comparably small amount of code.

3. **All Bridges Are Now Similarly Sized**: With signals-core as the shared reactive layer, no framework has an outsized advantage or disadvantage bridging it—Angular's `fromLoomSignal` and Lit's direct `observe()` call are both just a few lines, same as React's hook and Vue's composable.
...
6. **React and Vue are Most Popular**: Despite requiring comparable boilerplate to the others now, React and Vue are widely adopted and have large ecosystems.
...
This template defines empty containers that will be filled by the subscription handlers. When ViewModel signals change, the subscription code renders the appropriate card template into each container:
...
2. **ViewModel Fetches Data**: Each ViewModel makes an API request and updates its `data$` signal
3. **observe() Callback Fires**: The subscription handler receives the new data (and once immediately, at registration time, with whatever value was already there)
...
One critical aspect we haven't fully addressed is subscription cleanup. In the current implementation, `observe()` subscriptions are created in `subscribeToUpdates()` but never unsubscribed. For a long-running application, this could lead to memory leaks.
...
1. **Store Teardowns**: `observe()` returns a plain unsubscribe function—keep references to all of them in an array
2. **Cleanup Function**: Provide a function to call every stored teardown
...
- React: managed internally by `useSyncExternalStore`
- Vue: `onUnmounted` callback
- Angular: `DestroyRef.onDestroy`
...
**3. Direct Signal Subscriptions are Simple**
Calling `observe()` on a ViewModel signal is a single function call. Frameworks add abstractions (hooks, composables, bridge utilities) for convenience and cleanup management, but the core pattern is simple.
...
Memory leaks from uncleaned subscriptions are a real concern. Frameworks handle this automatically, but in vanilla JavaScript, you must manage it manually—store the unsubscribe function `observe()` returns and call it when you're done.
...
- **React** (Chapter 8): `useSignal` hook for ViewModel integration
- **Vue** (Chapter 9): Composition API with a `useSignal` composable
- **Angular** (Chapter 10): Dependency injection with the `fromLoomSignal` bridge
- **Lit** (Chapter 11): Web components with direct `observe()` calls
- **Vanilla JavaScript** (Chapter 12): Direct `observe()` subscriptions with EJS templates
...
- **Chapter 13: Reactive State Management Patterns** - Signals as the default, with RxJS as opt-in interop
...
**Next Chapter**: [Chapter 13: Reactive State Management Patterns](./chapter13.mdx) - Learn how `@web-loom/signals-core` signals and `store-core` enable reactive MVVM architecture, with RxJS covered as an optional interop layer.
