...
In the previous three chapters, we explored MVVM implementations in React, Vue, and Angular—three of the most popular frontend frameworks. Each required a small bridge to mirror `@web-loom/signals-core` signals into its own reactivity system: React used a hook, Vue used a composable, Angular used `fromLoomSignal`. Now we turn to Lit—a library that takes a fundamentally different approach by building on **web standards**.
...
Unlike React and Vue, Lit doesn't need a custom hook or composable at all. Instead, we call `observe()` directly from `@web-loom/signals-core` in `connectedCallback` and update `@state` properties, which automatically trigger re-renders.
...
```
`connectedCallback` is a web component lifecycle method called when the element is added to the DOM. We call `observe()` on the ViewModel's `data$` signal and update the `@state` property when a value arrives. `observe()` invokes the callback immediately with the current value, then again on every future change, and it returns a plain unsubscribe function—no `Subscription` object to manage.
...
```
`disconnectedCallback` is called when the element is removed from the DOM. We call the unsubscribe function to prevent memory leaks—the same pattern we saw in React and Vue.
...
## 11.3 Building the Sensor List: Direct Signal Subscriptions

The Sensor List component demonstrates a complete CRUD interface with Lit. Unlike Angular's `fromLoomSignal` bridge, Lit requires manual subscription management, but the pattern is straightforward and similar to React and Vue.
...
```
After subscribing to the data signal, we immediately execute the `fetchCommand` to load sensor data. This is the same pattern we saw in React and Vue.
...
**1. Teardown Functions Instead of Subscription Objects**:
```typescript
private teardowns: Array<() => void> = [];
```
Instead of tracking `Subscription` instances, we store plain unsubscribe functions in an array. This makes cleanup simpler when dealing with multiple signals.

**2. computed() for Coordinated State**:
```typescript
const anyLoading$ = computed(
  () =>
    greenHouseViewModel.isLoading$.get() ||
...
```
`@web-loom/signals-core`'s `computed()` combines multiple signals into a single derived signal—the direct replacement for RxJS's `combineLatest`. The dashboard shows a loading state if *any* ViewModel is loading. We then `observe()` the computed signal once to mirror its value into `@state`. This is a powerful pattern for coordinating state across multiple ViewModels, and it reads more directly than a `combineLatest([...]).subscribe(([a, b, c, d]) => ...)` chain—it's just a boolean expression.
...
```
When the component disconnects, we call every teardown function at once using `forEach()`. This ensures no memory leaks regardless of how many subscriptions we have.
...
Now that we've seen MVVM implementations in four frameworks, let's compare their approaches side by side. We'll focus on the key integration points: reading signals, triggering re-renders, and cleanup.

### 11.7.1 Signal Subscription Patterns
...
// - React manages subscribe/unsubscribe timing
```
...
<!-- fromLoomSignal internally:
     - Seeds a native Angular signal with sig.peek()
     - Subscribes via sig.subscribe(), tears down via DestroyRef
-->
```
...
|--------|-------|-----|---------|-----|
| **Integration** | `useSignal` hook | `useSignal` composable | `fromLoomSignal` bridge | Direct `observe()` call |
| **Boilerplate** | Low | Low | Low | Medium |
| **Lifecycle** | useEffect | onMounted/onUnmounted | ngOnInit + DestroyRef | connectedCallback/disconnectedCallback |
| **Re-render Trigger** | useSyncExternalStore | shallowRef.value = | native Angular signal `.set()` | @state property |
| **Cleanup** | Managed by React | onUnmounted | DestroyRef.onDestroy | disconnectedCallback |
...
// Exposes:
// - data$: ReadonlySignal<SensorListData>
...
connectedCallback() {
  super.connectedCallback();
  this.unsubscribe = observe(sensorViewModel.data$, (data) => {
    this.sensors = data;
  });
  sensorViewModel.fetchCommand.execute();
}
...
- `@web-loom/signals-core`'s `observe()`/`.subscribe()`, which are themselves plain functions with no library-specific ceremony
...
2. **Direct signal subscriptions**: Unlike Angular's `fromLoomSignal` bridge, Lit calls `observe()` directly in `connectedCallback` and cleans up in `disconnectedCallback`, similar in shape to React and Vue's bridges but without a hook/composable wrapper.
...
10. **Framework-agnostic MVVM**: Lit proves that MVVM architecture works beautifully with web standards—no framework required, just the platform and `@web-loom/signals-core`'s plain-function API.
