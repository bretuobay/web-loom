...
- **Exposing reactive state**: Publishing data changes through signals
...
1. **Signals**: The Model uses `signal()` (from `@web-loom/signals-core`) internally and exposes read-only `ReadonlySignal` views via `.asReadonly()`. This provides reactive state that any framework can subscribe to.
...
3. **Three core signals**: Every Model exposes `data$`, `isLoading$`, and `error$`. This consistent interface makes Models predictable and easy to consume.
...
5. **Lifecycle management**: The `dispose()` method marks the Model disposed, so its setters become no-ops — preventing memory leaks when Models are no longer needed.

## Framework Independence Through Signals

Why use `@web-loom/signals-core` signals instead of framework-specific reactivity (React state, Vue refs, Angular's own signals)?

**Signals are framework-agnostic.** Every major framework can consume them through a small bridge:

- **React**: Read via `useSyncExternalStore` in a `useSignal` hook, no local state duplication needed
- **Vue**: Bridge to a `shallowRef` via a `useSignal` composable
- **Angular**: Bridge to a native Angular Signal via `fromLoomSignal`
- **Vanilla JS**: Call `.subscribe()` or `observe()` directly
...
3. **Know about frameworks**: Models don't import React, Vue, or Angular. They're pure TypeScript classes with `@web-loom/signals-core`.
...
export class SensorModel extends RestfulApiModel<...> {
  ...
  getSensorsByType(type: SensorType): ReadonlySignal<SensorData[]> {
    return computed(() => this.data$.get()?.filter(s => s.type === type) ?? []);
  }
}
```
...
1. **Models are framework-agnostic**: They use `@web-loom/signals-core` signals, not framework-specific reactivity. This makes them reusable across React, Vue, Angular, and beyond.

2. **Models manage domain data**: They fetch from APIs, validate with Zod schemas, and expose reactive state through signals.

3. **BaseModel provides the foundation**: Core signals (`data$`, `isLoading$`, `error$`), validation, and lifecycle management.
