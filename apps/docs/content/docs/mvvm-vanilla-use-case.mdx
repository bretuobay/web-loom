...
summary: "How to wire Web Loom ViewModels into a framework-free TypeScript app — manual DOM rendering, @web-loom/signals-core signals as the only reactive layer, EJS templates, a client-side router, and imperative event listeners."
topicTitle: 'MVVM Core'
topicSlug: 'mvvm-core'
prevTitle: 'MVVM in Angular'
prevSlug: '/docs/mvvm-angular-use-case'
nextTitle: 'MVVM in React Native'
nextSlug: '/docs/mvvm-react-native-use-case'
---
...
This makes the vanilla case the most explicit illustration of how Web Loom's architecture actually works. The ViewModel is a plain TypeScript object with `@web-loom/signals-core` signals and Commands. Your code subscribes to those signals, turns the current data into HTML strings, and injects them into the DOM. There is no magic in between.
...
- **`subscriptions.ts`** — subscribes to ViewModel signals; updates state and re-renders on change
...
`subscriptions.ts` is the only place where ViewModel signals are consumed. It connects the reactive ViewModel layer to the imperative DOM layer. From `apps/mvvm-vanilla/src/app/subscriptions.ts`:
...
`observe(sig, fn)` — not plain `sig.subscribe(fn)` — is used throughout: it calls `fn` immediately with the signal's current value, then again on every future change. That matters here because `renderLayout()`/the card renderers need to run on the very first call too, not just on subsequent updates.

Each callback does the same three things:
1. Update the `state` snapshot
2. Check which view is currently active
3. Re-render only the relevant portion of the DOM

There is no diffing. A new value replaces the entire container's `innerHTML`. This is intentionally simple — in a small app the cost is negligible.
...
4. Command sets isExecuting$ = true, calls the API via the Model
         ↓
5. API responds — Model calls setData() with the new list
         ↓
6. greenHouseViewModel.data$ signal updates to the new list
         ↓
7. subscriptions.ts observer fires:
...
Subscriptions in `subscriptions.ts` run for the lifetime of the page and are intentionally never unsubscribed in the Greenhouse app — the page has no teardown. In a more complex app, or for testing, collect and dispose them. `observe()`/`sig.subscribe()` both return a plain unsubscribe function (no `Subscription` object), so a flat array of functions is all you need:
...
describe('GreenHouseViewModel', () => {
  it('starts with null data', () => {
    const data = greenHouseViewModel.data$.peek();
...
describe('subscribeToUpdates', () => {
  ...
  it('updates state.greenHouses when data$ changes', () => {
    const data = [{ id: '1', name: 'Alpine House', location: 'Zone A', size: '50sqm' }];
    (greenHouseViewModel.data$ as WritableSignal<any>).set(data);
...
**Do** subscribe to ViewModel signals before calling `fetchCommand.execute()`. `sig.subscribe()` only fires on future changes — if you subscribe after the fetch resolves, you miss that change entirely.

```typescript
// Correct order
subscribeToUpdates();       // subscriptions registered
await fetchCommand.execute(); // resulting change received by subscriber

// Wrong order
await fetchCommand.execute(); // data$ changes — nobody listening yet
...
**Don't** subscribe to the same signal multiple times. `subscribeToUpdates()` is called once in `main.ts`. Calling it again creates duplicate subscriptions that fire the same handlers twice.
...
**Don't** read from ViewModel signals imperatively with `.peek()` for rendering. Use `observe()` so the UI automatically updates when data changes.

```typescript
// Wrong — stale snapshot, won't update when data changes
const greenHouses = greenHouseViewModel.data$.peek();
app.innerHTML = renderGreenhouseList(greenHouses);

// Correct — observe and re-render on change (and on the initial value)
observe(greenHouseViewModel.data$, (greenHouses) => {
  state.greenHouses = greenHouses ?? [];
...
- [MVVM in React](/docs/mvvm-react-use-case) — React integration with `useSignal` and `useSyncExternalStore`
- [MVVM in Vue](/docs/mvvm-vue-use-case) — Vue 3 composable-based integration
- [MVVM in Angular](/docs/mvvm-angular-use-case) — Angular DI and signal bridging
