...
The `@web-loom/media-vue` package (published from the sibling [`web-loom-extensions`](https://github.com/bretuobay/web-loom-extensions) repo) shows a more advanced composable convention — accepting either a raw value or a `Ref<T>` as input:

```typescript
// @web-loom/media-vue — internal.ts
...
In `@web-loom/media-vue`'s `useMediaState.ts`, the `watch()` callback receives an `onCleanup` argument — a function to register cleanup logic that runs before the next invocation or on unmount:
