---
title: 'MVVM in Marko'
summary: 'How Marko combines server-streaming HTML with client-side reactivity, and how Web Loom ViewModels integrate via reactive let variables and a lightweight subscribeToObservable utility built on @web-loom/signals-core.'
...
The Web Loom Marko app uses the same ViewModels as every other framework demo — the integration requires only a thin utility to bridge `@web-loom/signals-core` signals to Marko's reactive state variables.
...
The Marko app includes a small utility that translates `@web-loom/signals-core` signals into Marko-compatible state updates. It kept its original RxJS-era name (`subscribeToObservable`) since Marko templates already reference it, but it's signal-based internally:
...
The `skipFirst` option skips delivering the signal's current value immediately, using plain `sig.subscribe()` (future changes only) instead of `observe()`. This is useful when the server has already rendered the initial state from a promise-based fetch and you only want to react to subsequent client-side changes.
...
<!-- Mount: subscribe to signals, return cleanup -->
<script>
...
```typescript
useEffect(() => {
  return vm.data$.subscribe(setValue);
}, []);
```

Or Vue's:
```typescript
const unsubscribe = observe(vm.data$, v => (ref.value = v));
onUnmounted(unsubscribe);
```

The pattern is universal — subscribe (or `observe`) on mount, call the returned unsubscribe function on unmount. Only the syntax differs.
...
```marko
<!-- Convert the current signal value to a Promise for SSR -->
...
The `observableToPromise` utility resolves the signal's *current* value (`.peek()`) as a Promise, which Marko's `<await>` tag can stream to the client progressively.
...
This is one of the places where Marko's architecture is genuinely different from the SPA frameworks — the server renders real HTML that the browser can paint before any JavaScript executes. Every Web Loom signal always holds a current value synchronously (readable via `.peek()`), the same way a `BehaviorSubject` did before the migration, making it compatible with this pattern.
...
it('data$ reflects the fetched list', () => {
...
- **Subscribe to signals** — `<script>` block (mount hook)
