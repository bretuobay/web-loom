...
```bash
npm install @web-loom/mvvm-patterns @web-loom/mvvm-core
```
...
The Interaction Request pattern solves this by inverting control: the ViewModel raises a request through an event stream (`EventSubscribable`, from `@web-loom/mvvm-core`), and the View subscribes to that stream and handles it using whatever UI components it chooses.
...
export function ItemView() {
  ...
  useEffect(() => {
    return vm.deleteRequest.requested$.subscribe(({ context, callback }) => {
...
onMounted(() => {
  unsubscribe = vm.deleteRequest.requested$.subscribe(({ context, callback }) => {
    const confirmed = window.confirm(`Delete "${context.itemName}"?`);
    callback({ ...context, confirmed });
  });
});
...
export class ItemComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.unsubscribe = this.vm.deleteRequest.requested$.subscribe(({ context, callback }) => {
...
`ActiveAwareViewModel` extends `BaseViewModel` with an `isActive` property and an `isActive$` signal, plus an `onIsActiveChanged` lifecycle hook you can override.

### Usage

`onIsActiveChanged` already fires on every transition, so the simplest polling implementation starts/stops an interval directly from that hook rather than composing it from `isActive$`:
...
export class DashboardViewModel extends ActiveAwareViewModel<DashboardModel> {
  ...
  protected override onIsActiveChanged(isActive: boolean): void {
    if (isActive) {
      this.model.refresh(); // immediately refresh when becoming active
      this.pollingHandle = setInterval(() => this.model.refresh(), 30_000);
    } else {
...
|--------|------|-------------|
| `requested$` | `EventSubscribable<InteractionRequestedEvent<T>>` | Fires when `raiseAsync` is called. Subscribe in the View; `.subscribe(fn)` returns a plain unsubscribe function. |
...
| Member | Type | Description |
|--------|------|-------------|
| `isActive` | `boolean` | Gets or sets the current active state. Setting it triggers `onIsActiveChanged` when the value actually changes. |
| `isActive$` | `ReadonlySignal<boolean>` | The reactive form of `isActive`. Signals only notify on actual changes, so no distinct-until-changed handling is needed. |
...
| `dispose()` | `void` | Marks the ViewModel disposed (further `isActive` writes become no-ops) and calls `super.dispose()`. |
...
- Start/stop side effects (polling, animation frames) directly from `onIsActiveChanged` rather than composing `isActive$` through ad-hoc subscriptions — the hook already fires exactly on each true/false transition.
