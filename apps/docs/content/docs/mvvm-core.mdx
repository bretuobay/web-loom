...
Each framework needs a small bridge to read a `ReadonlySignal<T>` reactively. See the dedicated [React](/docs/mvvm-react-use-case), [Vue](/docs/mvvm-vue-use-case), [Angular](/docs/mvvm-angular-use-case), [Solid](/docs/mvvm-solid-use-case), [Lit](/docs/mvvm-lit-use-case), [vanilla JS](/docs/mvvm-vanilla-use-case), [Marko](/docs/mvvm-marko-use-case), and [React Native](/docs/mvvm-react-native-use-case) pages for deep dives; the essentials for each are below.
...
`useSignal` mirrors a Web Loom signal into a Solid accessor. The two graphs stay separate — do not call loom `.get()` inside `createMemo`.
