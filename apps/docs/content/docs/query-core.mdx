...
`@web-loom/mvvm-core`'s source includes a `QueryStateModel`/`QueryStateModelView` pair intended to wrap Query Core endpoints inside viewmodels — mapping `endpointState` into `@web-loom/signals-core` signals (`data$`, `isLoading$`, `error$`) and exposing commands (`refetchCommand`, `invalidateCommand`) for UI actions. These are **not currently exported** from `mvvm-core`'s public entry point, so treat them as internal/unstable — see [Models: QueryStateModel](/docs/models#querystatemodel-internal-not-yet-exported).

Until they're exported, compose `BaseModel` with Query Core directly: subscribe to the endpoint's state inside a `BaseModel` subclass and call `setData`/`setLoading`/`setError` from that subscription, then call `refetch()`/`invalidate()` on the underlying `QueryCore` instance from your ViewModel's commands. This keeps the viewmodel as the boundary between UI components and the shared cache, while Query Core handles caching and deduplication.
