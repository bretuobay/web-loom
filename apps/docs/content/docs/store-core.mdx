...
Use `store-core` for UI state that should live beside your MVVM Core view models rather than inside a single Model signal. For example, a `TaskBoardViewModel` can create a small store to hold `statusFilter`, expose `store.actions` for filter buttons, and mirror the latest setting into a `@web-loom/signals-core` `signal()` that drives a `computed()` task-filtering pipeline.
...
export class CounterService implements OnDestroy {
  private _state = signal(counterStore.getState());
