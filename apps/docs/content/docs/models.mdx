...
The Model is the data layer of the MVVM pattern. It owns raw data, communicates with APIs, and exposes reactive signals that ViewModels read. A well-designed Model has no knowledge of the UI — no framework imports, no presentation logic.

`@web-loom/mvvm-core` ships model base classes that cover the most common data patterns, built on [`@web-loom/signals-core`](/docs/signals-core).
...
> `QueryStateModel<TData, TSchema>` (reading via a `@web-loom/query-core` cache) exists in `mvvm-core`'s source but isn't exported from its public entry point yet — see the note in the [QueryStateModel](#querystatemodel-internal-not-yet-exported) section below.

Both classes expose the same reactive signals and implement `IDisposable`.

---

## BaseModel

`BaseModel` is the foundation for every model in the system. It wraps three signals (from `@web-loom/signals-core`) and provides methods to update them safely.
...
Every `BaseModel` exposes three public signals:

- **`data$`** — `ReadonlySignal<TData | null>` holding the current value
- **`isLoading$`** — `ReadonlySignal<boolean>` for loading state
- **`error$`** — `ReadonlySignal<any>` for the last error

Read the current value synchronously with `.get()` (tracked, for use inside `computed()`/`effect()`) or `.peek()` (untracked). `.subscribe(fn)` only fires on *future* changes — it does not replay the current value to a new subscriber. If you need BehaviorSubject-style "deliver now, then on every change" semantics, use `observe(sig, fn)` from `@web-loom/signals-core` instead of `.subscribe()`.
...
Marks the model disposed: `setData`/`setLoading`/`setError`/`clearError` all become no-ops afterward, so nothing can push further updates through `data$`/`isLoading$`/`error$`. It does **not** call `.complete()` on anything (signals don't have that concept) and does not itself unsubscribe existing subscribers — subscribers simply stop receiving updates because nothing will call the setters again. Always call this when the model is no longer needed.
...
export class AuthModel extends RestfulApiModel<UserData, typeof UserSchema> {
  ...
  get isAuthenticated(): boolean {
    return this._token.peek() !== null;
  }

  get token(): string | null {
    return this._token.peek();
  }
}
```
...
## QueryStateModel (internal, not yet exported)

`mvvm-core`'s source includes a `QueryStateModel` that extends `BaseModel` to integrate with `@web-loom/query-core` for caching and stale-while-revalidate patterns — instead of managing its own HTTP calls, it would read from QueryCore's endpoint state (`refetch(force?)` to trigger a refetch, `invalidate()` to mark cached data stale). It is **not exported from `mvvm-core`'s public entry point**, so `import { QueryStateModel } from '@web-loom/mvvm-core'` does not currently compile — treat it as internal/unstable.

Until it's exported, compose `BaseModel` with `@web-loom/query-core` directly: subscribe to the QueryCore endpoint's state yourself inside a `BaseModel` subclass, calling `setData`/`setLoading`/`setError` from that subscription. See [Query Core](/docs/query-core) for the QueryCore API itself.
...
Constructor called
  ↓
Signals initialized with initialData
  ↓
...
Model sets isLoading$ = false
  ↓
ViewModel reads signals, derives state via computed(), renders
  ↓
Component unmounts → ViewModel.dispose() called
  ↓
Model disposed explicitly (see below) → setters become no-ops
```

`BaseViewModel.dispose()` does **not** automatically dispose an injected Model — it only disposes registered Commands and runs teardowns added via `addSubscription()` (there is no `addDisposable()` method). If your ViewModel constructs its own Model internally, dispose it explicitly in an overridden `dispose()`:

```typescript
class UserListViewModel extends BaseViewModel<GreenHouseModel> {
  constructor() {
    super(new GreenHouseModel());
  }

  dispose() {
    super.dispose();   // registered commands + addSubscription teardowns
    this.model.dispose();
  }
}
```

If the Model is injected from outside (the more common pattern — see [Don't: Create Models Inside the ViewModel](/docs/viewmodels#dont-create-models-inside-the-viewmodel)), dispose it wherever it was constructed instead.
...
describe('TestModel', () => {
  it('starts with null data and false loading', () => {
    const model = new TestModel();
    expect(model.data$.peek()).toBeNull();
    expect(model.isLoading$.peek()).toBe(false);
    model.dispose();
  });
...
describe('RestfulTodoListModel', () => {
  ...
  it('fetches and populates data$', async () => {
    ...
    expect(model.data$.peek()).toEqual(items);
    expect(model.isLoading$.peek()).toBe(false);
    model.dispose();
  });
  ...
  it('sets error$ on fetch failure', async () => {
    ...
    expect(model.error$.peek()).toBeInstanceOf(Error);
    model.dispose();
  });
...
`.subscribe()` only fires on *future* changes, not the value present when you subscribe — keep that in mind when asserting a sequence:

```typescript
it('sets loading=true then loading=false during fetch', async () => {
  const loadingStates: boolean[] = [];
  mockFetcher.mockResolvedValue([]);

  const model = createModel();
  const unsubscribe = model.isLoading$.subscribe(v => loadingStates.push(v));
  await model.fetch();
  unsubscribe();

  expect(loadingStates).toEqual([true, false]); // during, then after — no replay of the initial false
  model.dispose();
});
```
...
class TaskListViewModel extends BaseViewModel<TaskModel> {
  readonly incompleteTasks$ = computed(() => (this.data$.get() ?? []).filter((t) => !t.done));
}
```
...
### Do: Dispose Models You Construct Internally

```typescript
// ✅ Good — ViewModel owns the Model it created, so it disposes it explicitly
...
```typescript
// ❌ Bad — super.dispose() alone never touches the Model; setData/setLoading/
// setError silently keep working forever, and nothing marks it disposed
class UserViewModel extends BaseViewModel<UserModel> {
  constructor() {
    super(new UserModel());
  }
  // no dispose() override — model.dispose() never called
}
```
...
// ✅ Good
class UserViewModel extends BaseViewModel<UserModel> {
  readonly loadCommand = this.registerCommand(
    new Command(async () => {
...
- Read-heavy data shared across multiple ViewModels? See [QueryStateModel (internal, not yet exported)](#querystatemodel-internal-not-yet-exported) above for the current recommended approach.

---

## Where to Go Next

- [ViewModels](/docs/viewmodels) — how ViewModels consume Model signals, derive state, and expose commands
