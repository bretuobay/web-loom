---
id: "dumb-view-philosophy-and-view-layer-contracts"
title: The Dumb View Philosophy and View Layer Contracts
...
export class EnvironmentalMonitoringViewModel {
  ...
  private readonly statusSubject = new BehaviorSubject<EnvironmentalStatus>(
    "normal"
  );
  ...
  private async initialize(): Promise<void> {
    try {
      ...
      // Load initial data
      const initialReadings = await this.sensorService.getCurrentReadings(
        this.greenhouseId
      );
...
const viewModel = useViewModel(
  EnvironmentalMonitoringViewModel,
  sensorService,
  props.greenhouseId
);
...
export class GreenhouseManagementViewModel {
  ...
  private readonly selectedSubject = new BehaviorSubject<Greenhouse | null>(
    null
  );
  private readonly loadingSubject = new BehaviorSubject<boolean>(true);
  ...
  private readonly updateCommand = new Subject<{
    id: GreenhouseId;
  ...
  private async handleCreate(data: CreateGreenhouseDto): Promise<void> {
    ...
    this.greenhousesSubject.next([
      ...this.greenhousesSubject.value,
      optimisticGreenhouse,
    ]);
    ...
    try {
      ...
      const updated = this.greenhousesSubject.value.map((g) =>
        g.id === optimisticId ? created : g
      );
      this.greenhousesSubject.next(updated);
    } catch (error) {
      // Rollback optimistic update on failure
      const rolledBack = this.greenhousesSubject.value.filter(
        (g) => g.id !== optimisticId
      );
  ...
  private async handleUpdate(
    ...
    this.greenhousesSubject.next(
      this.greenhousesSubject.value.map((g) => (g.id === id ? optimistic : g))
    );

    try {
      const updated = await this.greenhouseService.update(id, data);
      this.greenhousesSubject.next(
        this.greenhousesSubject.value.map((g) => (g.id === id ? updated : g))
      );
    } catch (error) {
      // Rollback to original
      this.greenhousesSubject.next(
        this.greenhousesSubject.value.map((g) => (g.id === id ? original : g))
      );
      this.handleError(error);
    } finally {
      this.savingSubject.next(false);
...
const sortedGreenhouses = computed(() => {
  return [...state.value.greenhouses].sort((a, b) =>
...
export class AlertConfigurationViewModel {
  private readonly formSubject = new BehaviorSubject<AlertConfigForm>(
      ...
      <section class="form-section">
        <label class="toggle-label">
          <input
            ...
            @change="viewModel.toggleEnabled()"
          />
          ...
          <div class="form-field">
            <label>
              ...
              <input
                ...
                @input="
                  (e) =>
                    viewModel.updateThreshold('min', Number(e.target.value))
                "
          ...
          <div class="form-field">
            <label>
              ...
              <input
                ...
                @input="
                  (e) =>
                    viewModel.updateThreshold('max', Number(e.target.value))
                "
                :class="{ error: hasError('thresholds') }"
              />
            </label>
          ...
          <div class="form-field">
            <label>
              ...
              <input
                ...
                @input="
                  (e) =>
                    viewModel.updateThreshold('warning', Number(e.target.value))
                "
...
describe("EnvironmentalMonitoring", () => {
  ...
  beforeEach(() => {
    ...
    (EnvironmentalMonitoringViewModel as jest.Mock).mockImplementation(
      () => mockViewModel
    );
  });
