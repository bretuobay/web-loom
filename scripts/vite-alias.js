...
export var workspacePackages = __spreadArray(__spreadArray([], webLoomPackages, true), repoPackages, true);
 ...
 */
export function createAliases(dirname, additionalAliases) {
    if (additionalAliases === void 0) { additionalAliases = {}; }
    // Calculate the root path (assumes apps are in apps/ or packages are in packages/)
    var isInApps = dirname.includes('/apps/');
    var isInPackages = dirname.includes('/packages/');
    var rootPath = isInApps || isInPackages ? resolve(dirname, '../..') : dirname;
    var aliases = {
        '@': resolve(dirname, './src'),
    };
    // Add all workspace package aliases
    for (var _i = 0, _a = Object.entries(packagePaths); _i < _a.length; _i++) {
        var _b = _a[_i], pkg = _b[0], relativePath = _b[1];
        aliases[pkg] = resolve(rootPath, relativePath);
    }
    // Merge additional aliases
    return __assign(__assign({}, aliases), additionalAliases);
}
/**
 ...
 */
export function createAliasArray(dirname, additionalAliases) {
    if (additionalAliases === void 0) { additionalAliases = []; }
    var objectAliases = createAliases(dirname);
    var aliasArray = Object.entries(objectAliases).map(function (_a) {
    ...
    return __spreadArray(__spreadArray([], additionalAliases, true), aliasArray, true);
}
/**
 ...
 */
export function filterWorkspacePackages(usedPackages) {
    return usedPackages.filter(function (pkg) { return webLoomPackages.includes(pkg) || repoPackages.includes(pkg); });
}
/**
 * Common externals for library builds.
 * Libraries should externalize these to avoid bundling workspace dependencies.
 */
export var libraryExternals = __spreadArray(__spreadArray([
    'react',
    'react-dom',
    'react/jsx-runtime',
    'vue'
], webLoomPackages, true), repoPackages, true);
/**
 * Common globals for library builds (UMD format).
 */
export var libraryGlobals = {
    react: 'React',
    'react-dom': 'ReactDOM',
