...
interface Report {
  packages: Record<string, CategoryStats>;
  apps: Record<string, CategoryStats>;
  totals: {
...
function countLinesInFile(filePath: string): Omit<CodeStats, 'files'> {
...
function createEmptyStats(): CodeStats {
  return { files: 0, lines: 0, blanks: 0, comments: 0, code: 0 };
}

function addStats(a: CodeStats, b: Omit<CodeStats, 'files'>): void {
  a.files++;
  a.lines += b.lines;
  a.blanks += b.blanks;
...
function scanDirectory(dirPath: string, stats: CategoryStats): void {
  try {
...
function mergeStats(target: CodeStats, source: CodeStats): void {
...
function generateReport(rootDir: string): Report {
...
function printStats(label: string, stats: CodeStats): void {
...
if (process.argv.includes('--json')) {
  const fs = require('fs');
  const outputPath = join(rootDir, 'loc-report.json');
